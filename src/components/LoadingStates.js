...
export const LoadingSpinner = ({ size = 'large', color = '#1A889F', message }) => {
  ...
  return (
    <div className="flex-1 flex flex-col justify-center items-center p-4">
      ...
      />
      {message && <p className={`mt-3 text-sm ${isDarkMode ? 'text-[#b3b3b3]' : 'text-[#666]'}`}>{message}</p>}
    </div>
  );
};
...
export const SkeletonPlaceholder = ({ width = '100%', height = 16, className }) => {
  ...
  return (
    <div
      className={`${isDarkMode ? 'bg-[#2a2a2a]' : 'bg-[#E8E8E8]'} rounded animate-pulse ${className || ''}`}
      style={{ width, height }}
    />
  );
};
...
export const CardSkeleton = () => {
  const { isDarkMode } = useTheme();
  return (
    <div className={`${isDarkMode ? 'bg-[#181818] border border-[#4d4d4d]' : 'bg-white'} p-4 rounded-xl mx-4 my-2 shadow-sm`}>
...
export const ListItemSkeleton = () => {
  const { isDarkMode } = useTheme();
  return (
    <div className={`${isDarkMode ? 'bg-[#181818] border border-[#4d4d4d]' : 'bg-white'} p-4 mx-4 my-1 rounded-xl flex flex-row items-center shadow-sm`}>
...
export const EmptyState = ({ icon, title, message, actionButton, className }) => {
  ...
  return (
    <div className={`flex-1 flex flex-col justify-center items-center px-8 ${className || ''}`}>
      ...
      <h3 className={`text-lg font-bold mb-2 text-center ${isDarkMode ? 'text-white' : 'text-black'}`}>{title}</h3>
      {message && <p className={`text-sm text-center mb-6 leading-5 ${isDarkMode ? 'text-[#b3b3b3]' : 'text-[#666]'}`}>{message}</p>}
      {actionButton}
    </div>
  );
};
